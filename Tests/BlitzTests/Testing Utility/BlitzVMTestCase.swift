...
class BlitzVMTestCase: XCTestCase {
    let vm = VM()
    var printedValues: [Value] = []

    func run_source(_ source: String) {
        with_result_of_interpreting(source, do: { _, _ in })
    }

    func expect_source<T: Equatable>(_ source: String,
                       yields array: [T],
        ...
        with_result_of_interpreting(source, do: { result, _ in
            ...
            case let .value(value):
                switch value {
                case .array:
                    guard let arr = typecast<[T]>.any(Array<T>.fromBlitzValue(value)) else {
                        XCTFail("Failed converting \(value) to \(Array<T>.self)", file: file, line: line)
                        return
                    }
                    XCTAssertEqual(arr, array, file: file, line: line)

                default:
                    XCTFail("Type mismatch: expected Array, got \(value)", file: file, line: line)
                }
    ...
    func with_result_of_interpreting(_ source: String,
                                     ...
                                     line: UInt = #line) {
        do {
            vm.print = { [weak self] in self?.printedValues.append($0) }
            let program = try vm.parse(source)
